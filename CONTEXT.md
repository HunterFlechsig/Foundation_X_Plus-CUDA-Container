...
**Focused performance**:
The test-partition score of the task that was just trained, measured on the checkpoint written at the end of that epoch.
_Avoid_: training loss

**Unfocused performance**:
The test-partition score of a CANDID-PTX task that was not the task trained on that epoch, measured on that same checkpoint.
